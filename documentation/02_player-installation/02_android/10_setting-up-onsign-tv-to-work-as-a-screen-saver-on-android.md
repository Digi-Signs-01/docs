---
title: 'Setting Up Digisigns To Work As A Screen Saver On Android'
slug: setting-up-onsign-tv-to-work-as-a-screen-saver-on-android
publish: false
date: 2025-05-03
update: 2025-05-03
description: Setting Up Digisigns To Work As A Screen Saver On Android
categories:
  - start
---

# Setting up Digisigns to work as a Screen Saver on Android

While not widely known, a significant number of Android devices allow setting up a screen saver. As a result, it is also possible to set up Digisigns to work as a screen saver.
...
- To prevent Digisigns from being activated at boot, we recommend disabling the "Automatically start the application when the device starts"(more details on this below).
- The screen saver mode is compatible with [SPM](11_signage-platform-module-spm.md) (Signage Platform Manager) modules for generic Android devices and [SKM](09_samsung-kiosk-manager-skm.md) (Samsung Kiosk Manager). In all cases, it is always recommended to have these modules installed to enable advanced remote device management.
- The Digisigns screen saver does not implement any user restrictions. We emphasize this, as the user will have full Android access. Unless other restrictions are enforced on the device, the user can install and uninstall apps, or even disconnect the Digisigns Player from the account.
...
3. Install the [Digisigns Player App](https://app.onsign.tv/download/player/latest/android/) and [associate this Player](/basic-player-operations/connecting-a-player) with your Digisigns account.
...
5. After associating the Player with your Digisigns account, add some Content to the Player to test the display testing.
...
8. Choose the Signage Dream app to be displayed as the screen saver. Click on "Preview" to preview the Digisigns screen saver.
