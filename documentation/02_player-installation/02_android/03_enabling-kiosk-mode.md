...
Kiosk Mode is a feature available from Android 5 and up which locks the device to only run Digisigns. This is a security feature particularly useful for touch devices. It prevents users to access any other Android feature or setting.

**Once the Kiosk mode is configured, the device will:**

- Not allow users to leave the Digisigns  Player App
...
For maintenance, it is possible to set a password, so only authorized personal can exit the Digisigns  Player App. This is explained at the end of this tutorial.

**Samsung Devices:**

For **Samsung devices, Digisigns offers a different kiosk mode setup** with several advantages over the method described in this tutorial. Click [here](09_samsung-kiosk-manager-skm.md) to learn more.

**To configure your device to kiosk mode, you need:**

- A device running Android 5 or higher
- The Digisigns Android Player App 5.1.6 or higher
- To have ADB installed. (ADB is a command-line tool which lets you communicate with an Android device.) Learn [how to install ADB](04_installing-adb.md).
- Have your device set to run Digisigns as Device Owner App.

Please note, once Digisigns is configured as "Device Owner", the only way to remove it is by **performing a "Device Factory Reset"**.

**There are three steps required to configure your Android device to Kiosk Mode**

1. Factory reset
2. Install the Digisigns Player App
...
When the Developer Mode is enabled, it is time to use the ADB to install Digisigns and activate it as a Device Owner.

1. Download the latest Digisigns version from the [website](https://onsign.tv/downloads/).
2. Connect your device to your computer using a USB cable.
3. Open the Command Prompt and install Digisigns by typing the following command:  
   `adb install OnSignTV-N.N.N.apk`

OnSignTV-N.N.N.apk is the name of the file you downloaded. The N.N.N is a placeholder for the Digisigns version.
...
2. From this point on, Digisigns is active as the Device Owner and cannot be removed.
...
4. During association, you will be presented with an optional extra password field (Lock Password). This password is used for to exit the Digisigns Player App when running in Kiosk Mode.
...
If you did not set a password, the Digisigns Player Software will just exit without asking for a password.
