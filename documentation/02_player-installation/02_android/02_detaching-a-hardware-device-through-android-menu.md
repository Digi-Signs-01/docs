...
While it is always possible to detach a player through Digisigns platform, it is also possible to disconnect the hardware device directly on the Player side.

Follow these steps to detach de player on Android device:

1. If the Digisigns Player is running, right-click or press the Android back button to exit the Player. Uncheck the "Remain connected upon exit", as this will ensure the Player app is not running in background.
...
4. Select the account "Digisigns".
...
6. This will disconnect the player from your Digisigns, so the next time the Digisigns is loaded a new paring code will be shown.

As the hardware device is now detached, it can be connected to another player license, or even other Digisigns account.
