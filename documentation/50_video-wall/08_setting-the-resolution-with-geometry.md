...
This function is also useful when you use a computer as a Player and want to display it to the extended screen. Instead of manually adjusting the App window each time you open Digisigns, follow the instructions below and save time:

1. Open the Digisigns  Player App on your device.
...
5. The first two sets of numbers (X,Y) indicate where the top-left corner of the App begins, and the following two (L,H) define the desired resolution for the second screen or App. Confer the example below where one screen is used to access the Digisigns [website](https://onsign.tv/) and manage Content, while the larger one is used as the Player:
