---
title: 'Using China Proxy Server'
slug: using-china-proxy-server
publish: false
date: 2025-05-03
update: 2025-05-03
description: Using China Proxy Server
categories:
  - start
---

# Using China Proxy Server

Inside China, you can improve access speed with the Digisigns China proxy server.

1. Access your Digisigns account with [https://app.onsigntv.asia](https://app.onsigntv.asia/).
2. Connect your Players to the server in Asia following the instructions below.

## Connecting Digisigns Players to the Asia server

1. Before adding the Player to your Digisigns account, right click on the initial Player window and select the option Server Settings as indicated in the image below.  
   ![](https://static.helpjuice.com/helpjuice_production/uploads/upload/image/23821/direct/1731668796085/using-onsign-tv-china-proxy-server_1.png)
2. Select Digisigns (Asia) as the server.  
   ![](https://static.helpjuice.com/helpjuice_production/uploads/upload/image/23821/direct/1731668809364/using-onsign-tv-china-proxy-server_2.png)
3. Confirm with Save.
