...
- **Player requirements:**
  - **Windows, Android, or Linux** player -  Running Digisigns App version 10.3.0 or higher
  - **Samsung SSSP6** - Running Digisigns App 10.3.5 or above and Firmware 2430.3 or above
  - **LG webOS 6** - Running Digisigns App 10.3.5 or above and Firmware 03.68.33/1.6.8 or above

IMPORTANT: The Programmatic integration **will not** work in case the Digisigns App is outdated, or in case the SoC firmware is not matching the requirements.

The Digisigns Account must be enabled for the following features which are not enabled by default:
...
Configuring the Place Exchange App follows the same process as any other Digisigns content application.
...
1 - Configure the App name (like any other Digisigns App).
...
The Programmatic Plugin is generic, supporting all Digisigns integrated programmatic platforms. Only one instance needs to be added per player.
...
Digisigns can provide two different reports for the PlaceExchange App:
