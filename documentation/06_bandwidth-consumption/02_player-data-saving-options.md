...
Digisigns offers three options to configure on each Player to reduce the bandwidth usage:
...
The [Download Schedule](../31_player-download-settings/01_content-download-schedule.md) will also block the updates of:

- RSS,
- MRSS,
- Apps based on fetch_text.

[Download Schedule](../31_player-download-settings/01_content-download-schedule.md) will NOT block the updates of:
...
The [Content Update Interval](../31_player-download-settings/03_content-update-interval.md) will define how often your Player will check for updates. By default, the Player is set to "Realtime", meaning the Player will immediately synchronize when there is a Content update.

The [Content Update Interval](../31_player-download-settings/03_content-update-interval.md) can range from Realtime all the way up to 48 hours. Checking for updates less often will reduce the amount of data consumed by your Player.
